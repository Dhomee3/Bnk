...
export interface Account {
  ...
  craftedWeapons?: Record<string, number>;
}
...
export interface BotDB {
  ...
  settings: {
    ...
    weaponTransferLogChannelId?: string;
    [key: string]: any;
  };
}
...
function loadDB(): BotDB {
  ...
  for (const acc of Object.values(db.accounts)) {
    ...
    if (!acc.craftedWeapons) acc.craftedWeapons = {};
  }
  return db;
}
...
export function removeBalance(
  ...
  if (target === "cash") account.cash = (account.cash ?? 0) - amount;
...
export function bulkRemoveBalance(
  ...
  for (const account of Object.values(db.accounts)) {
    if (account.frozen) continue;
    const base = target === "cash" ? (account.cash ?? 0) : account.balance;
    const deducted = mode === "percent" ? Math.floor(base * value / 100) : Math.min(value, base);
...
export function deleteAccount(
  ...
  db.transactions.push({
    ...
    type: "transfer",
    fromAccount: accountId,
    description: `حذف الحساب ${accountId}`,
    timestamp: new Date().toISOString(),
    performedBy,
  });
  saveDB(db);
...
export function logRobbery(
  log: RobberyLog,
  payments: { accountId: string; amount: number; role: string }[]
): { results: { accountId: string; paid: boolean; error?: string }[] } {
  const db = loadDB();
  const results: { accountId: string; paid: boolean; error?: string }[] = [];
  let totalPaid = 0;
  for (const p of payments) {
    const acc = db.accounts[p.accountId];
    if (!acc) { results.push({ accountId: p.accountId, paid: false, error: "الحساب غير موجود" }); continue; }
    if (acc.frozen) { results.push({ accountId: p.accountId, paid: false, error: "الحساب مجمّد" }); continue; }
    acc.cash = (acc.cash ?? 0) + p.amount;
    totalPaid += p.amount;
    results.push({ accountId: p.accountId, paid: true });
  }
  log.totalPaid = totalPaid;
  db.robberyLogs.push(log);
  saveDB(db);
  return { results };
}
...
export function setWeaponStock(key: string, qty: number): void {
  const db = loadDB();
  if (!db.weaponStock) db.weaponStock = {};
  db.weaponStock[key] = qty;
  saveDB(db);
}

export function addResources(accountId: string, key: string, qty: number): void {
  const db = loadDB();
...
export function removeResources(
  ...
  if (!acc.inventory) acc.inventory = {};
  const current = acc.inventory[key] ?? 0;
  if (current < qty) return { success: false, error: `تملك ${current} فقط من هذا المورد` };
  acc.inventory[key] = current - qty;
  saveDB(db);
  return { success: true };
}
...
export function transferResources(
  ...
  const to = db.accounts[toId];
  if (!from) return { success: false, error: "حساب المُرسِل غير موجود" };
  if (!to) return { success: false, error: "حساب المُستلم غير موجود" };
  if (!from.inventory) from.inventory = {};
  if (!to.inventory) to.inventory = {};
  const current = from.inventory[key] ?? 0;
  if (current < qty) return { success: false, error: `تملك ${current} فقط من هذا المورد` };
  from.inventory[key] = current - qty;
...
export function buyResource(
  ...
  if (acc.balance < total) return { success: false, error: `رصيدك (${acc.balance.toLocaleString()}) غير كافٍ — المطلوب ${total.toLocaleString()}` };
  const stock = (db.weaponStock ?? {})[key] ?? 0;
  if (stock < qty) return { success: false, error: `المخزون المتاح (${stock}) أقل من الكمية المطلوبة` };
  acc.balance -= total;
...
export function addWeaponActivation(activation: WeaponActivation): void {
  const db = loadDB();
  if (!db.weaponActivations) db.weaponActivations = [];
  db.weaponActivations.push(activation);
  saveDB(db);
}
